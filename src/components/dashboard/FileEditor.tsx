...
export default function FileEditor({ file, onBack, profile }: FileEditorProps) {
  ...
  return (
    <motion.div 
      ...
      exit={{ opacity: 0, y: 10 }}
      className="h-full flex flex-col bg-white dark:bg-black overflow-hidden relative"
    >
      <header className="px-6 h-16 border-b border-neutral-100 dark:border-neutral-800 flex items-center justify-between bg-white dark:bg-black shrink-0">
        <div className="flex items-center gap-4 flex-1">
          <button onClick={onBack} className="p-2 -ml-2 hover:bg-neutral-50 dark:hover:bg-neutral-900 rounded-xl transition-all active:scale-95">
            <ArrowLeft className="w-5 h-5 text-neutral-400" />
          </button>
          <div className="flex items-center gap-3 flex-1">
            <button 
              onClick={() => fileInputRef.current?.click()}
              className="w-10 h-10 rounded-xl bg-neutral-50 dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 flex items-center justify-center shrink-0 overflow-hidden relative group"
            >
              {headerImage ? (
                <>
            ...
            <input
              ...
              className="outline-none bg-transparent w-full font-light text-lg tracking-tight text-neutral-900 dark:text-white placeholder:text-neutral-200 dark:placeholder:text-neutral-700"
        ...
        <div className="flex items-center gap-2">
          {isPublic ? (
            <div className="flex items-center bg-green-50 dark:bg-green-900/20 text-green-600 px-3 py-1.5 rounded-full text-xs font-medium mr-2">
              <Globe className="w-3 h-3 mr-1.5" />
              Published
              <button 
                onClick={(e) => { e.stopPropagation(); window.open(`/v/${file.ownerId}/${file.id}`, '_blank'); }}
                className="ml-2 hover:text-green-800 dark:hover:text-green-400"
              >
          ...
          <button
            ...
            className={cn(
              ...
              isPublic ? "bg-neutral-100 dark:bg-neutral-900 text-neutral-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-800" : "bg-neutral-100 dark:bg-neutral-900 text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-800 hover:text-black dark:hover:text-white"
            )}
          ...
          <button
            ...
            className={cn(
              "p-2.5 rounded-full transition-all flex items-center justify-center",
              hasUnsavedChanges && !saving ? "bg-primary text-white hover:scale-105 active:scale-95 shadow-lg shadow-primary/20" : "bg-neutral-50 dark:bg-neutral-900 text-neutral-300 dark:text-neutral-700 pointer-events-none"
            )}
            title="Save"
          >
            {saving ? <Loader2 className="w-4 h-4 animate-spin text-primary" /> : <Save className="w-4 h-4" />}
          </button>
        </div>
      </header>
      ...
      {editor && (
        <div className="sticky top-0 z-10 w-full bg-white/80 dark:bg-black/80 backdrop-blur-md border-b border-neutral-100 dark:border-neutral-800 px-6 py-2 flex items-center gap-1 overflow-x-auto no-scrollbar">
          <button onClick={() => editor.chain().focus().toggleBold().run()} className={cn("p-2 rounded-lg transition-colors", editor.isActive('bold') ? 'bg-neutral-100 dark:bg-neutral-800 text-black dark:text-white' : 'text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-neutral-900')}>
            <Bold className="w-4 h-4" />
          </button>
          <button onClick={() => editor.chain().focus().toggleItalic().run()} className={cn("p-2 rounded-lg transition-colors", editor.isActive('italic') ? 'bg-neutral-100 dark:bg-neutral-800 text-black dark:text-white' : 'text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-neutral-900')}>
            <Italic className="w-4 h-4" />
          </button>
          <div className="w-px h-4 bg-neutral-200 dark:bg-neutral-800 mx-2" />
          <button onClick={() => editor.chain().focus().toggleHeading({ level: 1 }).run()} className={cn("p-2 rounded-lg transition-colors", editor.isActive('heading', { level: 1 }) ? 'bg-neutral-100 dark:bg-neutral-800 text-black dark:text-white' : 'text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-neutral-900')}>
            <Heading1 className="w-4 h-4" />
          </button>
          <button onClick={() => editor.chain().focus().toggleHeading({ level: 2 }).run()} className={cn("p-2 rounded-lg transition-colors", editor.isActive('heading', { level: 2 }) ? 'bg-neutral-100 dark:bg-neutral-800 text-black dark:text-white' : 'text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-neutral-900')}>
            <Heading2 className="w-4 h-4" />
          </button>
          <div className="w-px h-4 bg-neutral-200 dark:bg-neutral-800 mx-2" />
          <button onClick={() => editor.chain().focus().toggleBulletList().run()} className={cn("p-2 rounded-lg transition-colors", editor.isActive('bulletList') ? 'bg-neutral-100 dark:bg-neutral-800 text-black dark:text-white' : 'text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-neutral-900')}>
            <List className="w-4 h-4" />
          </button>
          <button onClick={() => editor.chain().focus().toggleOrderedList().run()} className={cn("p-2 rounded-lg transition-colors", editor.isActive('orderedList') ? 'bg-neutral-100 dark:bg-neutral-800 text-black dark:text-white' : 'text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-neutral-900')}>
            <ListOrdered className="w-4 h-4" />
          </button>
          <button onClick={() => editor.chain().focus().toggleBlockquote().run()} className={cn("p-2 rounded-lg transition-colors", editor.isActive('blockquote') ? 'bg-neutral-100 dark:bg-neutral-800 text-black dark:text-white' : 'text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-neutral-900')}>
            <Quote className="w-4 h-4" />
          </button>
          <div className="w-px h-4 bg-neutral-200 dark:bg-neutral-800 mx-2" />
          <button onClick={() => editor.chain().focus().insertContent({ type: 'image', attrs: { src: '' } }).run()} className="p-2 rounded-lg transition-colors text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-50 dark:hover:bg-neutral-900" title="Insert Image">
            <ImageIcon className="w-4 h-4" />
          </button>
          <button onClick={() => editor.chain().focus().insertContent({ type: 'bentoGallery', attrs: { images: ['', '', ''] } }).run()} className="flex items-center gap-2 p-2 px-3 rounded-lg transition-colors bg-neutral-50 dark:bg-neutral-900 text-neutral-600 dark:text-neutral-400 hover:text-black dark:hover:text-white hover:bg-neutral-100 dark:hover:bg-neutral-800" title="Insert Gallery">
      ...
      <div className="flex-1 flex overflow-hidden">
            ...
            <div className="flex flex-wrap items-center gap-2 mb-10">
              ...
              <div className="relative">
                <button 
                  ...
                  className="w-7 h-7 rounded-lg bg-neutral-50 dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800 flex items-center justify-center text-neutral-400 hover:text-black dark:hover:text-white transition-colors"
                  title="Add tag"
                >
                ...
                <AnimatePresence>
                  {showTagMenu && (
                    <motion.div 
                      ...
                      className="absolute top-full left-0 mt-2 p-1.5 bg-white dark:bg-neutral-900 rounded-xl shadow-2xl border border-neutral-100 dark:border-neutral-800 z-50 flex items-center gap-2"
                    >
                      <input 
                        type="text" 
                        value={newTagName}
                        ...
                        placeholder="Tag..."
                        className="text-xs px-2 py-1.5 outline-none bg-neutral-50 dark:bg-neutral-800 rounded-lg w-24 text-neutral-900 dark:text-white"
                        onKeyDown={e => e.key === 'Enter' && addTag()}
                        autoFocus
                      />
                      <button onClick={addTag} className="p-1.5 bg-primary text-white rounded-lg"><Plus className="w-3.5 h-3.5" /></button>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
        ...
        {extractedImages.length > 0 && (
          <aside className="hidden lg:flex w-80 border-l border-neutral-50 dark:border-neutral-800 bg-neutral-50/20 dark:bg-neutral-900/20 flex-col overflow-hidden">
             <div className="p-6 border-b border-neutral-100 dark:border-neutral-800 bg-white/50 dark:bg-black/50 backdrop-blur-sm">
                <div className="px-2 py-1.5 bg-primary rounded-lg inline-flex items-center gap-2 mb-4">
                   <ImageIcon className="w-3 h-3 text-white" />
                   <span className="text-[9px] font-black uppercase tracking-[0.2em] text-white">Gallery</span>
                </div>
                <h3 className="text-xl font-bold text-neutral-900 dark:text-white tracking-tight">Gallery Summary</h3>
                <p className="text-xs text-neutral-400 dark:text-neutral-500 font-medium mt-1">All visual assets in this document</p>
             </div>
                ...
                <div className="columns-2 gap-3 space-y-3">
                   {extractedImages.map((src, i) => (
                     <motion.div 
                       ...
                       className="relative group rounded-xl overflow-hidden border border-neutral-100 dark:border-neutral-800 shadow-sm bg-white dark:bg-neutral-800 break-inside-avoid"
                     >
                       <img 
                          src={src} 
